...
function argumentsOf(values) {
  ...
  for (let index = 0; index < values.length; index += 2) {
    ...
    if (![...REQUIRED, OPTIONAL].includes(name) || Object.hasOwn(parsed, name) || typeof value !== "string" || !value) fail("transition producer arguments are invalid");
    parsed[name] = name === "action" ? value : resolve(value);
  }
...
async function main() {
  ...
  try {
    ...
    const before = fstatSync(directoryDescriptor); if (!before.isDirectory()) fail("transition output directory is invalid");
    const output = join(directory, name); descriptor = openSync(output, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | (constants.O_NOFOLLOW ?? 0), 0o600); created = true; writeFileSync(descriptor, bytes); fsyncSync(descriptor);
    const stored = fstatSync(descriptor), after = statSync(directory); if (!stored.isFile() || stored.size !== bytes.length || after.dev !== before.dev || after.ino !== before.ino) fail("transition receipt was not written exactly");
  } catch (error) {
    if (created) { try { unlinkSync(join(directory, name)); } catch { /* remove only this invocation's new output */ } }
    throw error;
  } finally {
...
try { await main(); }
catch { process.stderr.write("accepted transition target build failed\n"); process.exitCode = 1; }
