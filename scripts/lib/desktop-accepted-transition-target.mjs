...
function strictJSON(raw, label = "accepted transition target") {
  const result = spawnSync("/usr/bin/python3", ["-I", "-c", STRICT_JSON], { input: raw, encoding: "utf8", maxBuffer: MAX_INPUT });
  try { if (result.error || result.signal || result.status !== 0) fail(`${label} is malformed`); return JSON.parse(result.stdout); }
...
function protectedContext() {
  ...
  if (process.env.GITHUB_ACTIONS !== "true" || process.env.GITHUB_REPOSITORY !== REPOSITORY || process.env.GITHUB_REF !== "refs/heads/main" || process.env.GITHUB_WORKFLOW_REF !== `${WORKFLOW}@refs/heads/main` || process.env.RUNNER_ENVIRONMENT !== "github-hosted") fail("canonical protected-main workflow context is required");
  ...
  let descriptor;
  try { descriptor = openSync(workspace, constants.O_RDONLY | (constants.O_DIRECTORY ?? 0) | (constants.O_NOFOLLOW ?? 0)); if (!fstatSync(descriptor).isDirectory()) fail("canonical protected-main workspace is required"); }
  catch { fail("canonical protected-main workspace is required"); }
  finally { if (descriptor !== undefined) closeSync(descriptor); }
...
function targetRetainedAtHead(context, target) {
  ...
  const blob = git(context.workspace, ["show", `${context.head}:${name}`]);
  if (!Buffer.isBuffer(blob) || !blob.equals(target.bytes)) fail("protected target packet disagrees with verified evidence");
  return true;
}
...
function protectedPacket(path, context, label) {
  ...
  trackedAtHead(context, evidence, label);
  return evidence;
}
...
function protectedHistory(context, target, current, previous, requestedAction) {
  ...
  const index = strictJSON(indexEvidence.bytes, "protected release history");
  ...
  for (const packetEvidence of [target, current]) {
    ...
    if (!records.has(name)) {
      ...
      records.set(name, { name, position, evidence, declaration: strictJSON(evidence.bytes, "protected release declaration") });
    }
  ...
  if (targetRecord.position > currentRecord.position && targetBuild > currentBuild) action = targetRetained ? previous?.digest === target.digest ? "reupdate" : undefined : previous === null ? "update" : undefined;
  else if (targetRecord.position < currentRecord.position && targetBuild < currentBuild && previous?.digest === target.digest) action = "rollback";
...
function unchanged(evidence, label) {
  if (!boundedBytes(evidence.path, label, evidence.bytes.length).bytes.equals(evidence.bytes)) fail(`${label} changed during receipt production`);
}
...
export async function buildAcceptedDesktopTransitionTarget(input) {
  ...
  const verified = verifyRetainedDesktopAcceptedTargetEvidence({ artifactPath: values.artifactPath, packetPath: values.packetPath, bundlePath: values.bundlePath, releasePath: values.releasePath, tagRefPath: values.tagRefPath }), target = exactPacket(values.packetPath, "target packet");
  if (verified.repository !== REPOSITORY || verified.packetSHA256 !== target.digest || verified.packetFileName !== basename(target.path) || verified.artifactSourceSHA !== target.packet.sourceSHA || verified.releaseTag !== `neondiff-accepted-packet-${target.packet.tag}` || verified.artifactAttestationBundleSHA256 !== targetInputs[2].digest || verified.artifactAttestationBundleFileName !== basename(targetInputs[2].path)) fail("target packet is not independently verified");
  const current = protectedPacket(values.currentPacketPath, context, "current packet"), previous = values.previousTargetPacketPath === null ? null : protectedPacket(values.previousTargetPacketPath, context, "previous target packet"), history = protectedHistory(context, target, current, previous, values.action), tree = await buildExtractedAppTreeProof(values.artifactPath, target.packet.sourceSHA);
  ...
  authenticatedReceipts.add(receipt);
...
export function serializeAcceptedDesktopTransitionTarget(receipt) {
  if (!authenticatedReceipts.has(receipt)) fail("receipt was not produced by the accepted transition producer");
