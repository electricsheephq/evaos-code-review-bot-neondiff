...
export async function buildAcceptedDesktopReleasePacket(indexPath, artifactPath, feedPath, tagRefPath, tagObjectPath, releasePath) {
  for (const [value, label] of [[indexPath, "indexPath"], [artifactPath, "artifactPath"], [feedPath, "feedPath"], [tagRefPath, "tagRefPath"], [tagObjectPath, "tagObjectPath"], [releasePath, "releasePath"]]) if (typeof value !== "string" || !value) fail(`${label} must be a primitive path`);
  ...
  if (Object.keys(entry).length !== ENTRY_FIELDS.length || ENTRY_FIELDS.some((field) => !Object.hasOwn(entry, field)) || !/^(?:0|[1-9][0-9]*)$/.test(entry.length) || !Number.isSafeInteger(length) || length !== guarded.artifactBytes.length || entry.type !== "application/octet-stream" || entry.version !== declaration.version || entry.shortVersionString !== declaration.version || entry.build !== declaration.build || entry.channel !== "stable" || !/^\d+(?:\.\d+){1,2}$/.test(entry.minimumSystemVersion)) fail("selected appcast enclosure identity mismatch");
  const enclosure = buildFeedEnclosureProof({ url: entry.url, version: entry.version, build: entry.build, shortVersionString: entry.shortVersionString, channel: entry.channel, artifactName, artifactSHA256: guarded.artifactSHA256, edSignature: entry.edSignature }, { acceptedPublicKey: tree.bundleMarkers.publicKey, signedContent: guarded.artifactBytes });
