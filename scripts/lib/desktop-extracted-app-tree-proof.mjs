...
const SHA1 = /^[a-f0-9]{40}$/, SHA256 = /^[a-f0-9]{64}$/, TREE_KIND = "neondiff.desktop.extracted-tree-proof-v3";
...
result["sourceSHA"] = value.get("NeonDiffSourceSHA")
...
function plistMarkers(bytes, expectedSourceSHA) {
  ...
  if (typeof value.sourceSHA !== "string" || !SHA1.test(value.sourceSHA) || value.sourceSHA !== expectedSourceSHA) fail("bundle source identity is not canonical");
  ...
  return { appPath: "NeonDiff.app", bundleID, version, build, minimumSystemVersion, feedURL, publicKey, sourceSHA: value.sourceSHA, productionContract };
}
...
export async function buildExtractedAppTreeProof(artifactPath, sourceSHA) {
  ...
  return withMaterializedClassicZipApp(artifactPath, (appPath, graph) => {
    const { records, infoPlist } = appTree(appPath), proof = { schemaVersion: 3, kind: TREE_KIND, verified: true, algorithm: "sha256-tree-v1", sourceSHA, artifactSHA256: graph.artifactSHA256, artifactByteLength: graph.artifactByteLength, treeSHA256: treeDigest(records), records, bundleMarkers: plistMarkers(infoPlist, sourceSHA), appleDouble: { policy: "artifact-bound-excluded-from-tree-v1", entryCount: graph.records.filter((record) => typeof record.sidecarTarget === "string").length } };
