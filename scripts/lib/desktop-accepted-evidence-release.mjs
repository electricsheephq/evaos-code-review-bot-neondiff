...
function exactPacket(path, stableOnly) {
  const evidence = boundedBytes(path, "accepted packet", MAX_PACKET_BYTES), packet = parseAcceptedDesktopReleasePacket(evidence.bytes);
  if (packet.artifactByteLength > MAX_ARTIFACT_BYTES || stableOnly && (packet.channel !== "stable" || packet.version !== "1.1.0" || packet.tag !== STABLE_TAG)) fail("accepted packet identity is not canonical");
...
function exactArtifact(path, packet) {
  const artifact = boundedBytes(path, "accepted artifact", MAX_ARTIFACT_BYTES);
