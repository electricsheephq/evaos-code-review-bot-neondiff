...
try { const values = parseArguments(process.argv.slice(2)), receipt = verifyRetainedDesktopAcceptedEvidence({ artifactPath: values.artifact, packetPath: values.packet, bundlePath: values.bundle, releasePath: values.release, tagRefPath: values["tag-ref"] }); process.stdout.write(`${JSON.stringify(receipt)}\n`); }
