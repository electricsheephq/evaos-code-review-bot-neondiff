...
export function validateDesktopReleaseManifest(manifest, options = {}) {
  ...
  if (manifest.source.ref !== `sha:${manifest.source.commit}`) errors.push("source: ref must be the exact commit SHA ref");
  ...
  if (["beta", "stable"].includes(manifest.releaseLevel)) for (const name of gates) if (manifest[name].state !== "proven") errors.push(`${name}: released manifest requires proof`);
