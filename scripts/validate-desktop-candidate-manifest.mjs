...
function hasNegativeProofBoundary(items) { const negative = /\b(?:not|no|without|pending|unproven|excluded|never|cannot|can['’]?t|won['’]?t|isn['’]?t|doesn['’]?t)\b/i, subject = /\b(?:candidate|artifact|signed|signing|notarized|notarization|stapled|stapling|gatekeeper|feed|site|billing|customer|runtime|rollback)\b/i; return items.some((item) => negative.test(item) && subject.test(item)); }
...
export function validateDesktopReleaseManifest(manifest, options = {}) {
  ...
  if (released) { for (const name of gates) if (manifest[name].state !== "proven") errors.push(`${name}: released manifest requires proof`); if (hasNegativeProofBoundary(manifest.proofBoundary.excludes)) errors.push("proof boundary: released proof cannot be excluded"); }
