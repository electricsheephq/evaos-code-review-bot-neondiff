...
| Anthropic, OpenAI, and Gemini native BYOK adapters | `compatible by interface` | Env-gated single-provider review-fixture smoke; explicit review selection shipped behind proof gates | Explicit selection shipped, but rollout still requires dry-run/release evidence; native baseUrl overrides are disabled during beta | Hosted provider receives the smoke prompt, and receives prompts/diffs only if explicitly selected for review after proof | Provider-specific quality evidence, terms/privacy review, duplicate-suppression proof, and release-status evidence |
