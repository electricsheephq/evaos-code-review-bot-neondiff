...
| License activation | Public open-source repository review is free (no NeonDiff Activation Key; provider verification still applies); supported private, internal, and unknown-visibility review/provider work requires live API-backed activation. v1.0.4 grants no offline cache authority. The file backend is the supported CLI path; Desktop useful actions remain blocked until a native broker is proven. |
