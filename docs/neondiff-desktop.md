...
The desktop model now lives in the importable `NeonDiffDesktopAppCore` library target, the app executable depends on that library, and Core/AppCore behavior runs as real Swift Testing targets through `apps/neondiff-desktop/scripts/run-swift-tests.sh`. The wrapper uses the Apple-signed `Testing.framework` bundled with Command Line Tools when SwiftPM does not discover it automatically. Full Xcode, a genuinely hosted XCUITest target, and `.xcresult` evidence remain separate follow-up proof under #516; Command Line Tools builds are not hosted-UI evidence.
