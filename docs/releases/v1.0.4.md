...
- Public configuration now describes `publicRepoMode: activation-required`;
  the retired `free-source-available-beta` mode is not a supported use path.
...
- Installed proof: fresh status `missing/source=none`, provider work denied before
  input, API activation `active/source=api`, provider route HTTP 200 after
  activation, cache-only provider denial HTTP 403, one redacted browser Verify
  API Key result, and zero browser-console warnings or errors
- Evidence:
...
- Rollback reference: source baseline `refs/tags/v1.0.3` in an isolated recovery
  checkout. npm channel rollback or quarantine repair must use the reviewed
  release workflow; do not republish immutable versions or mutate dist-tags as
  an ad hoc shortcut.
