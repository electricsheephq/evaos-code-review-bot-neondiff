...
- The initial publish workflow
  [29185873762](https://github.com/electricsheephq/evaos-code-review-bot-neondiff/actions/runs/29185873762)
  published the exact immutable package, but its promotion confirmation gate failed
  before registry convergence. Recovery PR
  [#554](https://github.com/electricsheephq/evaos-code-review-bot-neondiff/pull/554)
  ran the idempotent recovery workflow
  [29190031396](https://github.com/electricsheephq/evaos-code-review-bot-neondiff/actions/runs/29190031396),
  verified the existing package without republishing, confirmed
  `latest=1.0.4`, and removed the owned `release-candidate` quarantine tag.
...
- [#561](https://github.com/electricsheephq/evaos-code-review-bot-neondiff/issues/561)
...
- `release-status --public-release-manifest docs/public-release-manifest.json --expected-public-version v1.0.4`
  intentionally reports the public-release update-channel gate as blocked,
  with the CLI, daemon, and browser dashboard channels marked `[BLOCKED]`,
  because v1.0.4 has no authorized npm predecessor-rollback command. The
  top-level runtime status can also be blocked by local runtime conditions.
  This is the expected fail-closed public-release state until
  [#559](https://github.com/electricsheephq/evaos-code-review-bot-neondiff/issues/559)
  ships; it does not negate the verified GitHub/npm publication state above.
