...
- Current state: NeonDiff is a source-available beta; desktop update channels
  are represented by `signed-appcast-desktop-post-launch` with `requiredForV1: false`
  in `docs/public-release-manifest.json`. The native source has a Sparkle 2
  controller and Release configuration gate, but no signed-feed or installed-update proof.
...
- Validation / eval gates:
  ...
  - Baseline/comparison: current non-required `signed-appcast-desktop-post-launch` manifest stage.
...
- release notes naming source commit, version, artifact identity, and rollback
  target
- public-safe evidence packet under `$RUN_DIR/`
