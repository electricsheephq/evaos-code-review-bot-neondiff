...
describe("beta release status", () => {
  ...
  function npmProof(overrides: Record<string, unknown> = {}): Record<string, unknown> {
    return {
      schemaVersion: 1,
      evidenceKind: "neondiff.npm-publication-proof.v1",
      ...
      proofBoundary: "npm-registry-publication-only",
      ...overrides
    };
  }

  function writeNpmProof(root: string, proof: unknown, path = npmProofPath): string {
    mkdirSync(dirname(join(root, path)), { recursive: true });
    writeFileSync(join(root, path), JSON.stringify(proof));
    return path;
  }

  it("accepts each publication identity mode and rejects hostile proof mutations", () => {
    const root = mkdtempSync(join(tmpdir(), "npm-publication-proof-"));
    roots.push(root);
    const validate = (proofPath: string, expectedTagCommit = npmTagCommit) => validateNpmPublicationProof({
      ...
      expectedTagCommit, expectedArtifact: npmArtifact, expectedReleaseCandidatePresent: false, now: new Date("2026-08-31T00:00:00.000Z")
    });
    ...
    for (const [name, overrides] of hostile) {
      expect(validate(writeNpmProof(root, npmProof(overrides), `docs/evidence/hostile-${name}.json`)).ok, name).toBe(false);
    }
    expect(validate("../outside.json").ok).toBe(false);
    expect(validate(writeNpmProof(root, "not-json", "docs/evidence/malformed.json")).ok).toBe(false);
    expect(validate("docs/evidence/missing.json").ok).toBe(false);
    expect(validate(writeNpmProof(root, npmProof(), "docs/evidence/missing-expected-tag.json"), "").ok).toBe(false);
  });
  ...
  it("keeps a pending candidate non-published and requires proof for published status", () => {
    ...
    expect(pending.npmPublication.requiredForThisRelease).toBe(true);
    expect(["candidate_pending_publication", "published"]).toContain(pending.npmPublication.state);
    const root = mkdtempSync(join(tmpdir(), "npm-publication-status-"));
    roots.push(root);
    mkdirSync(join(root, "docs", "releases"), { recursive: true });
    mkdirSync(join(root, "docs", "release-candidates"), { recursive: true });
    writeFileSync(join(root, "docs", "SETUP.md"), "# Setup\n");
    writeFileSync(join(root, "docs", "releases", "v1.0.5.md"), "# v1.0.5\n");
    writeChangelogHead(root, "1.0.5");
    ...
    writeFileSync(join(root, "public-release.json"), JSON.stringify({
      version: "v1.0.5", releaseLevel: "source-beta", packageArtifact: npmArtifact,
      docs: { version: "v1.0.5", setupPath: "docs/SETUP.md", releaseNotesPath: "docs/releases/v1.0.5.md" },
    ...
    writeFileSync(candidatePath, JSON.stringify(candidateFixture));
    expect(readPublicReleaseManifestStatus({ cwd: root, manifestPath: "public-release.json", expectedVersion: "v1.0.5", now: new Date("2026-08-31T00:00:00.000Z") }).npmPublication.ok).toBe(false);
    candidateFixture.state = "candidate_pending_publication";
    candidateFixture.registry.state = "pending_publication";
    ...
    delete candidateFixture.packageArtifact.shasum;
    writeFileSync(candidatePath, JSON.stringify(candidateFixture));
    const missingShasum = readPublicReleaseManifestStatus({ cwd: root, manifestPath: "public-release.json", expectedVersion: "v1.0.5" });
    expect(missingShasum.npmPublication).toMatchObject({ ok: false, candidateReadyForPublication: false });
    expect(missingShasum.npmPublication.detail).toContain("packageArtifact.shasum");
    candidateFixture.packageArtifact.shasum = npmArtifact.shasum;
    delete candidateFixture.registry.predecessor;
    writeFileSync(candidatePath, JSON.stringify(candidateFixture));
  ...
  it("separates recovered history including preserved authorized posts from active work", () => {
    ...
    processed.run("owner/repo", 13, "preserved-failure", "failed", "provider timeout", "2026-08-22T23:30:00.100Z");
    processed.run("owner/repo", 13, "preserved-blocking", "posted", null, "2026-08-22T23:00:00Z");
    processed.run("owner/repo", 14, "receipt-before-failure", "failed", "provider timeout", "2026-08-22T23:30:00.900Z");
    processed.run("owner/repo", 14, "prior-blocking", "posted", null, "2026-08-22T23:00:00Z");
    db.exec(`update processed_reviews set event = 'REQUEST_CHANGES' where pull_number in (13,14) and status = 'posted';
      insert into review_event_authorization_consumptions values ('owner/repo',13,'COMMENT','2026-08-22T23:30:00.900Z'),('owner/repo',14,'COMMENT','2026-08-22T23:30:00.100Z');`);
    db.exec(`with recursive n(x) as (values(1) union all select x+1 from n where x<10000) insert into processed_reviews (repo,pull_number,head_sha,status,error,created_at) select 'history/repo',x,'history-'||x,'posted',null,'2026-01-01T00:00:00Z' from n;
      with recursive n(x) as (values(1) union all select x+1 from n where x<100) insert into processed_reviews (repo,pull_number,head_sha,status,error,created_at) select 'error/repo',x,'error-'||x,'failed','provider timeout','2026-08-22T23:00:00Z' from n;`);
    db.exec("insert into processed_reviews (repo,pull_number,head_sha,status,error,created_at) values ('owner/repo',10,'outside-cutoff','failed','provider timeout','2026-08-22T00:00:00.100Z'),('owner/repo',11,'exact-cutoff','failed','provider timeout','2026-08-22T07:00:00.500+07:00'),('owner/repo',12,'inside-cutoff','failed','provider timeout','2026-08-22T00:00:00.900Z')");
    ...
    queue.run("preserved", "preserved", "automatic", "background", "owner/repo", "owner", 13, "preserved-failure", 1, "failed", "ordinary failure", "2026-08-22T23:30:00.100Z", "2026-08-22T23:30:00.100Z");
    queue.run("prior-receipt", "prior-receipt", "automatic", "background", "owner/repo", "owner", 14, "receipt-before-failure", 1, "failed", "ordinary failure", "2026-08-22T23:30:00.900Z", "2026-08-22T23:30:00.900Z");
    db.close();
    ...
    expect(status.database).toMatchObject({
      errorCount: 108,
      recentUnrecoveredErrorCount: 104,
      lastErrorAt: "2026-08-22T23:45:00.100Z",
      failedReviewQueueJobCount: 5,
