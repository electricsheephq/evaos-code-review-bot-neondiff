...
const persistentState = () => { const records = new Map<string, any>(); return { ...state(), getIssueEnrichmentRecord: (repo: string, issueNumber: number) => records.get(`${repo}#${issueNumber}`), recordIssueEnrichment: (value: any) => { const { now: _now, ...stored } = value; records.set(`${stored.repo}#${stored.issueNumber}`, stored); return stored; } }; };
...
describe("issue-enrichment lazy source settlement", () => {
  ...
  it("keeps repository identity stable across case-variant allowlist order", async () => {
    const sharedState = persistentState(), posted: number[] = []; prepareBranchWorktree.mockResolvedValue({ path: "/tmp/repo", headSha: "a".repeat(40) }); const github = { getRepo: async () => ({ default_branch: "main" }), listIssuesForEnrichment: async () => [issue(1)], getIssueOrPull: async () => issue(1), canPostAsApp: () => true, upsertIssueComment: async ({ issueNumber }: { issueNumber: number }) => { posted.push(issueNumber); return { action: "created" as const, id: issueNumber }; } }; for (const allowlist of [["Owner/Repo", "owner/repo"], ["owner/repo", "Owner/Repo"]]) await runIssueEnrichmentCycle({ config: config(allowlist), state: sharedState, github, dryRun: false, includeExisting: true, checkedAt: "2026-08-26T10:00:00.000Z", licenseAdmission: admission }); expect(posted).toEqual([1]); const scanned: string[] = []; await runIssueEnrichmentCycle({ config: config(["z/repo", "a/repo"]), state: state(), github: { ...github, listIssuesForEnrichment: async (repo: string) => { scanned.push(repo); return []; } }, dryRun: false, includeExisting: true, checkedAt: "2026-08-26T10:00:00.000Z", licenseAdmission: admission }); expect(scanned).toEqual(["z/repo", "a/repo"]);
  });
