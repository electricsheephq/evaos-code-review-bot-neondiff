...
try {
  const code = "import sys,zipfile; z=zipfile.ZipFile(sys.argv[1],'w');\nfor n,b,m in [('NeonDiff.app/Contents/file','payload',0o100644),('NeonDiff.app/Resources/Current',sys.argv[2],0o120777)]:\n i=zipfile.ZipInfo(n); i.external_attr=m<<16; z.writestr(i,b.encode())\nz.close()";
  execFileSync("/usr/bin/python3", ["-I", "-c", code, artifact, "../Contents/file"]);
  ...
  const escaping = join(root, "escaping.zip"); execFileSync("/usr/bin/python3", ["-I", "-c", code, escaping, "../../outside"]);
  assert.throws(() => withExtractedDesktopApp({ artifactPath: escaping }, () => null));
