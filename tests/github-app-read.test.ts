...
describe("GitHub App read authentication", () => {
  ...
  it("listIssueLabelEvents reads only the newest five pages from trusted pagination metadata", async () => {
    ...
    const contradiction = await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970); expect(contradiction).toMatchObject({ pagesRead: 1, lastPage: 2, terminal: "event_history_unbounded", overflow: true });
    globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")); return jsonResponse(page === 1 ? Array.from({ length: 100 }, (_unused, id) => ({ id })) : [], 200, "", page === 2 ? { Link: '<https://evil.invalid/repos/owner/repo/issues/970/events?per_page=100&page=2>; rel="last"' } : {}); }) as typeof fetch; expect(await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).toMatchObject({ terminal: "event_history_unbounded", overflow: true });
    globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")); return jsonResponse(Array.from({ length: page === 1 ? 100 : 50 }, (_unused, id) => ({ id: page * 100 + id }))); }) as typeof fetch; expect(await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).toMatchObject({ terminal: "event_history_unbounded", overflow: true });
    globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")), link = '<https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=1>; rel="first", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=1>; rel="prev", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=2>; rel="last"'; return jsonResponse(Array.from({ length: page === 1 ? 100 : 0 }, (_unused, id) => ({ id })), 200, "", page === 2 ? { Link: link } : {}); }) as typeof fetch; expect(await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).toMatchObject({ terminal: "event_history_unbounded", overflow: true });
    globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")), link = '<https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=1>; rel="first", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=1>; rel="prev", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=2>; rel="last"'; return jsonResponse(Array.from({ length: 100 }, (_unused, id) => ({ id: page * 100 + id })), 200, "", page === 2 ? { Link: link } : {}); }) as typeof fetch; expect(await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).toMatchObject({ length: 200, terminal: "complete", truncated: false });
    globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")), length = page === 4 ? 50 : 100, link = page < 8 ? `<https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=${page + 1}>; rel="next", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=8>; rel="last"` : '<https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=1>; rel="first", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=7>; rel="prev"'; return jsonResponse(Array.from({ length }, (_unused, id) => ({ id: page * 100 + id })), 200, "", { Link: link }); }) as typeof fetch; expect(await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).toMatchObject({ pagesRead: 2, rawCount: 50, terminal: "event_history_unbounded", overflow: true }); globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")); if (page === 4) throw new Error("tail unavailable"); return jsonResponse(Array.from({ length: 100 }, (_unused, id) => ({ id })), 200, "", { Link: '<https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=2>; rel="next", <https://api.github.com/repos/owner/repo/issues/970/events?per_page=100&page=8>; rel="last"' }); }) as typeof fetch; await expect(new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 970)).rejects.toThrow("tail unavailable");
  });
});
