...
describe("GitHub App read authentication", () => {
  ...
  it("counts only asynchronously qualified issues toward the bounded pagination minimum", async () => { const pages: number[] = []; globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")); pages.push(page); return jsonResponse([1, 2].map((offset) => ({ number: (page - 1) * 2 + offset, state: "open" }))); }) as typeof fetch; const result = await new GitHubApi({ token: "fixture" }).listIssuesForEnrichment("owner/repo", { perPage: 2, pageLimit: 3, minIssueResults: 2, qualifyForMinimum: async (issue) => issue.number > 2 }); expect(pages).toEqual([1, 2]); expect(result).toHaveLength(4); expect(result.scanCompletion).toBe("stopped_after_min_issue_results"); });
});
