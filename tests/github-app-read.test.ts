...
describe("GitHub App read authentication", () => {
  ...
  it("listIssueLabelEvents returns an array-compatible newest-200 receipt from raw Link headers", async () => { const pages: number[] = [], endpoint = "https://api.github.com/repos/owner/repo/issues/991/events"; globalThis.fetch = vi.fn(async (url) => { if (String(url) === "https://api.github.com/repos/owner/repo") return jsonResponse({ id: 42, full_name: "owner/repo", private: true }); const match = /\/issues\/991\/events\?per_page=100&page=(\d+)$/.exec(String(url)); if (!match) return jsonResponse({ message: "unexpected" }, 404); const page = Number(match[1]), count = page === 3 ? 50 : 100; pages.push(page); const members = page === 1 ? [[2, "next"], [3, "last"]] : page === 2 ? [[1, "prev"], [3, "next"], [3, "last"]] : [[2, "prev"], [3, "last"]]; return new Response(JSON.stringify(Array.from({ length: count }, (_unused, index) => ({ id: page * 1000 + index, event: "labeled" }))), { status: 200, headers: { "Content-Type": "application/json", Link: members.map(([target, relation]) => `<${endpoint}?page=${target}&per_page=100>; rel="${relation}"`).join(", ") } }); }) as typeof fetch; const result = await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 991); expect(pages).toEqual([1, 2, 3]); expect(result).toHaveLength(200); expect(result).toMatchObject({ pagesRead: 3, rawCount: 250, uniqueCount: 250, duplicateCount: 0, lastPage: 3, terminal: "bounded_tail", truncated: true, overflow: false }); expect(result.items[0]?.id).toBe(1050); });
