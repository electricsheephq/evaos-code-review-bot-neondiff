...
describe("GitHub App read authentication", () => {
  ...
  it("listIssueLabelEvents returns an array-compatible newest-200 receipt from raw Link headers", async () => { const pages: number[] = [], endpoint = "https://api.github.com/repos/owner/repo/issues/991/events"; let metadataReads = 0; globalThis.fetch = vi.fn(async (url) => { if (String(url) === "https://api.github.com/repos/owner/repo") { metadataReads += 1; return jsonResponse({ id: 42, full_name: "owner/repo", private: true }); } const match = /\/issues\/991\/events\?per_page=100&page=(\d+)$/.exec(String(url)); if (!match) return jsonResponse({ message: "unexpected" }, 404); const page = Number(match[1]), count = page === 3 ? 50 : 100; pages.push(page); const members = page === 1 ? [[2, "next"], [3, "last"]] : page === 2 ? [[1, "prev"], [3, "next"], [3, "last"]] : [[2, "prev"], [3, "last"]]; return new Response(JSON.stringify(Array.from({ length: count }, (_unused, index) => ({ id: page * 1000 + index, event: "labeled" }))), { status: 200, headers: { "Content-Type": "application/json", Link: members.map(([target, relation]) => `<${endpoint}?page=${target}&per_page=100>; rel="${relation}"`).join(", ") } }); }) as typeof fetch; const github = new GitHubApi({ token: "fixture" }), result = await github.listIssueLabelEvents("owner/repo", 991); await github.listIssueLabelEvents("owner/repo", 991); expect(metadataReads).toBe(1); expect(pages).toEqual([1, 2, 3, 1, 2, 3]); expect(result).toHaveLength(200); expect(result).toMatchObject({ pagesRead: 3, rawCount: 250, uniqueCount: 250, duplicateCount: 0, lastPage: 3, terminal: "bounded_tail", truncated: true, overflow: false }); expect(result.items[0]?.id).toBe(1050); });

  it("listIssueLabelEvents fails closed on a Link outside the exact event endpoint", async () => { globalThis.fetch = vi.fn(async (url) => { if (String(url) === "https://api.github.com/repos/owner/repo") return jsonResponse({ id: 42, full_name: "owner/repo", private: true }); if (String(url).includes("/issues/991/events?")) return new Response(JSON.stringify(Array.from({ length: 100 }, (_unused, id) => ({ id }))), { status: 200, headers: { "Content-Type": "application/json", Link: '<https://evil.example/issues/991/events?page=2&per_page=100>; rel="next", <https://evil.example/issues/991/events?page=3&per_page=100>; rel="last"' } }); return jsonResponse({ message: "unexpected" }, 404); }) as typeof fetch; const result = await new GitHubApi({ token: "fixture" }).listIssueLabelEvents("owner/repo", 991); expect(result).toMatchObject({ length: 0, pagesRead: 1, rawCount: 100, terminal: "event_history_unbounded", truncated: true, overflow: true }); });

  it("stops the in-page fold immediately after the async qualified minimum", async () => { const pages: number[] = [], qualified: number[] = []; globalThis.fetch = vi.fn(async (url) => { const page = Number(new URL(String(url)).searchParams.get("page")); pages.push(page); return jsonResponse([1, 2, 3].map((offset) => ({ number: (page - 1) * 3 + offset, state: "open" }))); }) as typeof fetch; const result = await new GitHubApi({ token: "fixture" }).listIssuesForEnrichment("owner/repo", { perPage: 3, pageLimit: 3, minIssueResults: 2, qualifyForMinimum: async (issue) => { qualified.push(issue.number); return issue.number > 3; } }); expect(pages).toEqual([1, 2]); expect(qualified).toEqual([1, 2, 3, 4, 5]); expect(result.map(({ number }) => number)).toEqual([1, 2, 3, 4, 5]); expect(result.scanCompletion).toBe("stopped_after_min_issue_results"); });
});
