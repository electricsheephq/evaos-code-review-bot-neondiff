...
`); chmodSync(fakeGh, 0o755);
  ...
  async function transitionAuthorityFixture(action: "update" | "rollback" | "reupdate") {
    ...
    return { action, baseIndexPath: join(base, "index.json"), currentPacketPath, evidence, environment, head, history, indexPath, output, previousTargetPacketPath, run, selectedPacket, targets, workspace };
  }
  ...
  it("derives protected update, rollback, and exact re-update authority", async () => {
    for (const action of ["update", "rollback", "reupdate"] as const) {
      ...
      const retainedPacketPath = join(value.targets, output.targetPacketFileName), retainedReceiptPath = join(value.targets, output.targetReceiptFileName); if (retainedPacketPath !== value.currentPacketPath && retainedPacketPath !== value.previousTargetPacketPath) writeFileSync(retainedPacketPath, value.evidence.packetBytes); else expect(readFileSync(retainedPacketPath)).toEqual(value.evidence.packetBytes); writeFileSync(retainedReceiptPath, receiptBytes); expect(spawnSync(process.execPath, ["scripts/validate-desktop-release-declaration.mjs", "--index", value.indexPath, "--base-index", value.baseIndexPath], { cwd: process.cwd(), encoding: "utf8" }).status).toBe(0);
    }
  }, 60_000);
  ...
  it("rejects re-update relabeling when the previous-target selector is omitted", async () => {
    const omitted = await transitionAuthorityFixture("reupdate"); expect(omitted.run("update", (values) => values.slice(0, -2)).status).not.toBe(0);
  }, 30_000);
