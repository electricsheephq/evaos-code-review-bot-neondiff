...
describe("canonical accepted Desktop release packet", () => {
  ...
  function packetFixture(sidecar = false, inAppSidecar = false) {
    ...
    return { paths: [indexPath, artifactPath, feedPath, tagRefPath, tagObjectPath, releasePath, acceptedPublicKeyPath] as const, root, artifactPath, privateKey, acceptedPublicKey, indexPath, feedPath, tagRefPath, tagObjectPath, releasePath, acceptedPublicKeyPath, declarationPath, declaration, tagRef, tagObject, release };
  }
  async function transitionFixture(action: "update" | "rollback" | "reupdate") {
    const value = packetFixture(), stablePacket = await buildAcceptedDesktopReleasePacket(...value.paths), stableDigest = acceptedDesktopReleasePacketDigest(stablePacket), betaVersion = "1.1.0-beta.87", betaBuild = "11087", betaTag = `v${betaVersion}`, betaName = `NeonDiff-${betaVersion}-build${betaBuild}-macOS.zip`, betaArtifactPath = join(value.root, betaName);
    writeFileSync(betaArtifactPath, classicZip([{ name: "NeonDiff.app/", type: "directory" }, { name: "NeonDiff.app/Contents/Info.plist", data: plist(betaVersion, "", value.acceptedPublicKey).replace("<string>11091</string>", `<string>${betaBuild}</string>`) }, { name: "NeonDiff.app/Contents/MacOS/NeonDiffDesktop", data: "desktop", mode: 0o100755 }]));
    const betaArtifact = readFileSync(betaArtifactPath), betaSHA = createHash("sha256").update(betaArtifact).digest("hex"), betaURL = `https://github.com/electricsheephq/evaos-code-review-bot-neondiff/releases/download/${betaTag}/${betaName}`, betaSignature = sign(null, betaArtifact, value.privateKey).toString("base64"), betaFeedPath = join(value.root, "beta-appcast.xml");
    writeFileSync(betaFeedPath, `<?xml version="1.0"?><rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle"><channel><title>NeonDiff Desktop beta</title><link>${betaFeed}</link><description>NeonDiff Desktop beta appcast</description><item><title>NeonDiff ${betaVersion}</title><pubDate>Sun, 24 Aug 2026 00:00:00 +0000</pubDate><sparkle:minimumSystemVersion>14.0</sparkle:minimumSystemVersion><sparkle:channel>beta</sparkle:channel><enclosure url="${betaURL}" length="${betaArtifact.length}" type="application/octet-stream" sparkle:version="${betaBuild}" sparkle:shortVersionString="${betaVersion}" sparkle:minimumSystemVersion="14.0" sparkle:edSignature="${betaSignature}" /></item></channel></rss>`);
    const betaFeedRaw = readFileSync(betaFeedPath), betaTree = await buildExtractedAppTreeProof(betaArtifactPath, sourceSHA), betaEntry = { url: betaURL, length: betaArtifact.length, type: "application/octet-stream", version: betaVersion, build: betaBuild, shortVersionString: betaVersion, minimumSystemVersion: "14.0", channel: "beta", edSignature: betaSignature }, betaEnclosure = buildFeedEnclosureProof({ url: betaURL, version: betaVersion, build: betaBuild, shortVersionString: betaVersion, channel: "beta", artifactName: betaName, artifactSHA256: betaSHA, edSignature: betaSignature }, { acceptedPublicKey: value.acceptedPublicKey, signedContent: betaArtifact });
    const betaRaw = Buffer.from(`${JSON.stringify({ schemaVersion: 1, kind: "neondiff.desktop.accepted-release-packet-v1", verified: true, channel: "beta", version: betaVersion, build: betaBuild, tag: betaTag, sourceSHA, tagObjectSHA, artifactURL: betaURL, artifactName: betaName, artifactByteLength: betaArtifact.length, artifactSHA256: betaSHA, treeSHA256: betaTree.treeSHA256, feedSHA256: createHash("sha256").update(betaFeedRaw).digest("hex"), feedEntry: betaEntry, enclosureProofSHA256: feedEnclosureProofDigest(betaEnclosure), npmReleaseClass: "paid-beta" })}\n`), betaPacket = parseAcceptedDesktopReleasePacket(betaRaw), betaDigest = createHash("sha256").update(betaRaw).digest("hex"), betaDeclarationPath = join(value.root, "declarations", `${betaTag}.json`);
    const betaDeclaration = { ...value.declaration, version: betaVersion, tag: betaTag, channel: "beta", sequence: "87", build: betaBuild, contract: "paid-mac-beta-byo-v1", distribution: { ...value.declaration.distribution, artifactName: betaName, releaseClass: "paid-beta", origins: { ...value.declaration.distribution.origins, feed: betaFeed } } }; writeFileSync(betaDeclarationPath, JSON.stringify(betaDeclaration)); writeFileSync(value.declarationPath, JSON.stringify({ ...value.declaration, predecessor: `${betaTag}.json` })); writeFileSync(value.indexPath, JSON.stringify({ schemaVersion: 1, status: "retained", declarationDirectory: "declarations", declarationPaths: [`${betaTag}.json`, `${tag}.json`], currentPath: `${tag}.json` }));
    const selected = action === "rollback" ? betaPacket : stablePacket, selectedRaw = action === "rollback" ? betaRaw : Buffer.from(serializeAcceptedDesktopReleasePacket(stablePacket)), selectedDigest = action === "rollback" ? betaDigest : stableDigest, current = action === "rollback" ? stablePacket : betaPacket, currentDigest = action === "rollback" ? stableDigest : betaDigest, acceptedDirectory = join(value.root, "accepted-targets"), artifactPath = action === "rollback" ? betaArtifactPath : value.artifactPath, feedPath = action === "rollback" ? betaFeedPath : value.feedPath; mkdirSync(acceptedDirectory);
    const acceptedTarget = { packetSHA256: selectedDigest, sourceSHA: selected.sourceSHA, tagObjectSHA: selected.tagObjectSHA, sparklePublicKeySHA256: createHash("sha256").update(Buffer.from(value.acceptedPublicKey, "base64")).digest("hex") }, targetRaw = Buffer.from(`${JSON.stringify({ schemaVersion: 1, kind: "neondiff.desktop.accepted-transition-target-v1", acceptedTarget, current: { tag: current.tag, build: current.build, artifactSHA256: current.artifactSHA256, treeSHA256: current.treeSHA256, packetSHA256: currentDigest }, previouslyAcceptedTargetPacketSHA256: action === "update" ? null : selectedDigest })}\n`), packetPath = join(acceptedDirectory, `${selectedDigest}.packet.json`), targetPath = join(acceptedDirectory, `${createHash("sha256").update(targetRaw).digest("hex")}.target.json`);
    writeFileSync(packetPath, selectedRaw); writeFileSync(targetPath, targetRaw);
    return { input: { action, indexPath: value.indexPath, packetPath, artifactPath, feedPath, targetPath }, value, packetPath, targetPath, betaDeclarationPath, selected, selectedDigest };
  }
  function rewritePacket(fixture: Awaited<ReturnType<typeof transitionFixture>>, raw: Buffer) { const packetSHA256 = createHash("sha256").update(raw).digest("hex"), packetPath = join(dirname(fixture.packetPath), `${packetSHA256}.packet.json`); writeFileSync(packetPath, raw); fixture.packetPath = packetPath; fixture.input.packetPath = packetPath; return packetSHA256; }
  function rewriteTarget(fixture: Awaited<ReturnType<typeof transitionFixture>>, value: object | string) { const raw = Buffer.from(typeof value === "string" ? value : `${JSON.stringify(value)}\n`), targetPath = join(dirname(fixture.targetPath), `${createHash("sha256").update(raw).digest("hex")}.target.json`); writeFileSync(targetPath, raw); fixture.targetPath = targetPath; fixture.input.targetPath = targetPath; }
  it("derives one frozen exact-source/artifact/tree/feed/enclosure/npm packet", async () => {
    ...
    const parsed = parseAcceptedDesktopReleasePacket(Buffer.from(serializeAcceptedDesktopReleasePacket(packet))); expect(parsed).toEqual(packet); expect(Object.isFrozen(parsed) && Object.isFrozen(parsed.feedEntry)).toBe(true); expect(() => serializeAcceptedDesktopReleasePacket(parsed)).toThrow("not produced");
    for (const raw of [Buffer.from(serializeAcceptedDesktopReleasePacket(packet).trim()), Buffer.from(serializeAcceptedDesktopReleasePacket(packet).replace('"kind":', '"kind":"duplicate","kind":')), Buffer.from(serializeAcceptedDesktopReleasePacket(packet).replace('"verified":true', '"verified":true,"extra":false'))]) expect(() => parseAcceptedDesktopReleasePacket(raw)).toThrow();
  ...
  it("emits redacted non-mutating update, rollback, and exact re-update plans", async () => {
    for (const action of ["update", "rollback", "reupdate"] as const) { const fixture = await transitionFixture(action), watched = [fixture.packetPath, fixture.input.artifactPath, fixture.input.feedPath, fixture.input.targetPath, fixture.input.indexPath], before = watched.map((path) => readFileSync(path)); const plan = await planAcceptedDesktopTransition(fixture.input); expect(Object.keys(plan)).toEqual(["action", "target", "packetSHA256", "nextSteps"]); expect(plan).toMatchObject({ action, target: { tag: fixture.selected.tag, version: fixture.selected.version, build: fixture.selected.build, channel: fixture.selected.channel }, packetSHA256: fixture.selectedDigest }); expect(plan.nextSteps).toEqual(["quiescence-and-entitlement-gates", "stage-exact-target", "atomic-transition", "poststate-and-feed-proof"]); expect(JSON.stringify(plan)).not.toMatch(/signature|publicKey|path|receipt|prestate|customer/i); if (action === "update") { const command = [join(process.cwd(), "scripts/desktop-update-command.mjs"), action, "--index", fixture.input.indexPath, "--packet", fixture.input.packetPath, "--artifact", fixture.input.artifactPath, "--feed", fixture.input.feedPath, "--target", fixture.input.targetPath]; expect(JSON.parse(execFileSync(process.execPath, command, { encoding: "utf8" }))).toEqual(plan); expect(() => execFileSync(process.execPath, command.map((value) => value === "--index" ? "index" : value), { stdio: "pipe" })).toThrow(); } expect(watched.map((path) => readFileSync(path))).toEqual(before); }
  });
  it("fails closed on packet, target, declaration, artifact, feed, and transition substitutions", async () => {
    const cases: Array<(fixture: Awaited<ReturnType<typeof transitionFixture>>) => void> = [
      (fixture) => { const target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.acceptedTarget.packetSHA256 = `sha256:${fixture.selectedDigest}`; rewriteTarget(fixture, target); },
      (fixture) => { const raw = Buffer.from(readFileSync(fixture.packetPath, "utf8").trim()), packetSHA256 = rewritePacket(fixture, raw), target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.acceptedTarget.packetSHA256 = packetSHA256; rewriteTarget(fixture, target); },
      (fixture) => rewriteTarget(fixture, readFileSync(fixture.targetPath, "utf8").replace('"kind":', '"kind":"duplicate","kind":')),
      (fixture) => writeFileSync(fixture.input.feedPath, `${readFileSync(fixture.input.feedPath, "utf8")} `),
      (fixture) => writeFileSync(fixture.input.artifactPath, "substituted"),
      (fixture) => { const declaration = JSON.parse(readFileSync(fixture.value.declarationPath, "utf8")); declaration.build = "11092"; declaration.distribution.artifactName = "NeonDiff-1.1.0-build11092-macOS.zip"; writeFileSync(fixture.value.declarationPath, JSON.stringify(declaration)); },
      (fixture) => { const target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.current = { tag: fixture.selected.tag, build: fixture.selected.build, artifactSHA256: fixture.selected.artifactSHA256, treeSHA256: fixture.selected.treeSHA256, packetSHA256: fixture.selectedDigest }; rewriteTarget(fixture, target); },
      (fixture) => { const feed = readFileSync(fixture.input.feedPath, "utf8").replaceAll("14.0", "13.0"); writeFileSync(fixture.input.feedPath, feed); const packet = { ...fixture.selected, feedSHA256: createHash("sha256").update(feed).digest("hex"), feedEntry: { ...fixture.selected.feedEntry, minimumSystemVersion: "13.0" } }, packetSHA256 = rewritePacket(fixture, Buffer.from(`${JSON.stringify(packet)}\n`)), target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.acceptedTarget.packetSHA256 = packetSHA256; rewriteTarget(fixture, target); },
      (fixture) => { const packet = { ...fixture.selected, sourceSHA: "3".repeat(40), tagObjectSHA: "4".repeat(40) }, packetSHA256 = rewritePacket(fixture, Buffer.from(`${JSON.stringify(packet)}\n`)), target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.acceptedTarget.packetSHA256 = packetSHA256; rewriteTarget(fixture, target); }
    ];
    for (const mutate of cases) { const fixture = await transitionFixture("update"); mutate(fixture); await expect(planAcceptedDesktopTransition(fixture.input)).rejects.toThrow(); }
    for (const action of ["rollback", "reupdate"] as const) { const fixture = await transitionFixture(action), target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.previouslyAcceptedTargetPacketSHA256 = "0".repeat(64); rewriteTarget(fixture, target); await expect(planAcceptedDesktopTransition(fixture.input)).rejects.toThrow(/previously accepted/i); }
    const race = await transitionFixture("update"), pending = planAcceptedDesktopTransition(race.input); writeFileSync(race.input.indexPath, `${readFileSync(race.input.indexPath, "utf8")} `); await expect(pending).rejects.toThrow(/changed during preflight/i);
  }, 20_000);
});
