...
describe("canonical accepted Desktop release packet", () => {
  ...
  async function transitionFixture(action: "update" | "rollback" | "reupdate") {
    ...
    const selected = action === "rollback" ? betaPacket : stablePacket, selectedRaw = action === "rollback" ? betaRaw : Buffer.from(serializeAcceptedDesktopReleasePacket(stablePacket)), selectedDigest = action === "rollback" ? betaDigest : stableDigest, current = action === "rollback" ? stablePacket : betaPacket, currentDigest = action === "rollback" ? stableDigest : betaDigest, packetPath = join(value.root, "accepted-packet.json"), targetPath = join(value.root, "accepted-target.json"), artifactPath = action === "rollback" ? betaArtifactPath : value.artifactPath, feedPath = action === "rollback" ? betaFeedPath : value.feedPath;
    writeFileSync(packetPath, selectedRaw); writeFileSync(targetPath, `${JSON.stringify({ schemaVersion: 1, kind: "neondiff.desktop.accepted-transition-target-v1", current: { tag: current.tag, build: current.build, artifactSHA256: current.artifactSHA256, treeSHA256: current.treeSHA256, packetSHA256: currentDigest }, previouslyAcceptedTargetPacketSHA256: action === "update" ? null : selectedDigest })}\n`);
    return { input: { action, indexPath: value.indexPath, packetPath, packetSHA256: selectedDigest, artifactPath, feedPath, targetPath, targetSHA256: createHash("sha256").update(readFileSync(targetPath)).digest("hex") }, value, packetPath, targetPath, betaDeclarationPath, selected, selectedDigest };
  }
  ...
  it("fails closed on packet, target, declaration, artifact, feed, and transition substitutions", async () => {
    const cases: Array<(fixture: Awaited<ReturnType<typeof transitionFixture>>) => void> = [
      (fixture) => { fixture.input.packetSHA256 = `sha256:${fixture.selectedDigest}`; },
      (fixture) => { fixture.input.targetSHA256 = `sha256:${fixture.input.targetSHA256}`; },
      (fixture) => writeFileSync(fixture.packetPath, readFileSync(fixture.packetPath, "utf8").trim()),
      (fixture) => { writeFileSync(fixture.targetPath, readFileSync(fixture.targetPath, "utf8").replace('"kind":', '"kind":"duplicate","kind":')); fixture.input.targetSHA256 = createHash("sha256").update(readFileSync(fixture.targetPath)).digest("hex"); },
      ...
      (fixture) => { const target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.current = { tag: fixture.selected.tag, build: fixture.selected.build, artifactSHA256: fixture.selected.artifactSHA256, treeSHA256: fixture.selected.treeSHA256, packetSHA256: fixture.selectedDigest }; writeFileSync(fixture.targetPath, `${JSON.stringify(target)}\n`); fixture.input.targetSHA256 = createHash("sha256").update(readFileSync(fixture.targetPath)).digest("hex"); }
    ];
    ...
    for (const action of ["rollback", "reupdate"] as const) { const fixture = await transitionFixture(action), target = JSON.parse(readFileSync(fixture.targetPath, "utf8")); target.previouslyAcceptedTargetPacketSHA256 = "0".repeat(64); writeFileSync(fixture.targetPath, `${JSON.stringify(target)}\n`); fixture.input.targetSHA256 = createHash("sha256").update(readFileSync(fixture.targetPath)).digest("hex"); await expect(planAcceptedDesktopTransition(fixture.input)).rejects.toThrow(/previously accepted/i); }
  }, 20_000);
});
