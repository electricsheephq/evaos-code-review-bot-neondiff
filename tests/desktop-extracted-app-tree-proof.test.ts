...
describe("canonical accepted Desktop release packet", () => {
  ...
  async function transitionAuthorityFixture(action: "update" | "rollback" | "reupdate" = "reupdate") {
    ...
    const selected = action === "rollback" ? betaPacket : packet, selectedPacketSHA256 = action === "rollback" ? betaPacketSHA256 : packetSHA256, selectedPacketPath = action === "rollback" ? betaPacketPath : acceptedPacketPath, selectedFeedPath = action === "rollback" ? betaFeedPath : value.feedPath, current = action === "rollback" ? { tag: packet.tag, build: packet.build, artifactSHA256: packet.artifactSHA256, treeSHA256: packet.treeSHA256, packetSHA256 } : { tag: betaTag, build: betaBuild, artifactSHA256: betaArtifactSHA256, treeSHA256: betaTree.treeSHA256, packetSHA256: betaPacketSHA256 }; let selectedArtifactPath = action === "rollback" ? betaArtifactPath : acceptedArtifactPath;
    ...
    let targetEvidencePacketPath = acceptedPacketPath, targetBundlePath = bundlePath, targetReleasePath = evidenceReleasePath, targetTagRefPath = evidenceTagRefPath; if (action === "rollback") { const targetEvidenceTag = `neondiff-accepted-packet-${selected.tag}`, targetEvidenceRoot = join(value.root, "target-evidence-download"); mkdirSync(targetEvidenceRoot); selectedArtifactPath = join(targetEvidenceRoot, selected.artifactName); targetEvidencePacketPath = join(targetEvidenceRoot, `${selectedPacketSHA256}.packet.json`); writeFileSync(selectedArtifactPath, readFileSync(betaArtifactPath)); writeFileSync(targetEvidencePacketPath, betaPacketBytes); const targetStatement = { ...statement, subject: [{ name: selected.artifactName, digest: { sha256: selected.artifactSHA256 } }], predicate: { ...statement.predicate, releaseTag: selected.tag, artifactSourceSHA: selected.sourceSHA, acceptedPacketSHA256: selectedPacketSHA256 } }, targetBundleBytes = Buffer.from(JSON.stringify({ mediaType: "application/vnd.dev.sigstore.bundle.v0.3+json", verificationMaterial: { tlogEntries: [{}] }, dsseEnvelope: { payload: Buffer.from(JSON.stringify(targetStatement)).toString("base64"), payloadType: "application/vnd.in-toto+json", signatures: [{ sig: Buffer.alloc(64, 1).toString("base64") }] } })), targetBundleSHA256 = createHash("sha256").update(targetBundleBytes).digest("hex"), targetBundleName = `${targetBundleSHA256}.artifact-source-attestation.json`; targetBundlePath = join(targetEvidenceRoot, targetBundleName); writeFileSync(targetBundlePath, targetBundleBytes); targetReleasePath = join(value.root, "target-evidence-release.json"); targetTagRefPath = join(value.root, "target-evidence-tag-ref.json"); const targetAsset = (name: string, bytes: Buffer, digest: string) => ({ name, size: bytes.length, digest: `sha256:${digest}`, browser_download_url: `https://github.com/${repository}/releases/download/${targetEvidenceTag}/${name}` }); writeFileSync(targetReleasePath, JSON.stringify({ tag_name: targetEvidenceTag, name: `NeonDiff accepted packet evidence ${selected.tag}`, draft: false, prerelease: true, immutable: true, target_commitish: selected.sourceSHA, html_url: `https://github.com/${repository}/releases/tag/${targetEvidenceTag}`, assets: [targetAsset(basename(targetEvidencePacketPath), betaPacketBytes, selectedPacketSHA256), targetAsset(targetBundleName, targetBundleBytes, targetBundleSHA256)] })); writeFileSync(targetTagRefPath, JSON.stringify({ ref: `refs/tags/${targetEvidenceTag}`, object: { type: "commit", sha: selected.sourceSHA } })); }
    const fakeGh = join(value.root, "gh"); writeFileSync(fakeGh, `#!/usr/bin/env node\nconst {readFileSync}=require("node:fs"),args=process.argv.slice(2);if(args[0]!=="attestation")process.stdout.write("{}\\n");else{const document=JSON.parse(readFileSync(args[args.indexOf("--bundle")+1],"utf8")),statement=JSON.parse(Buffer.from(document.dsseEnvelope.payload,"base64").toString("utf8"));process.stdout.write(JSON.stringify([{verificationResult:{statement}}])+"\\n");}\n`); chmodSync(fakeGh, 0o755);
    const input = { action, acceptedArtifactPath, acceptedPacketPath, acceptedBundlePath: bundlePath, acceptedReleasePath: evidenceReleasePath, acceptedTagRefPath: evidenceTagRefPath, targetArtifactPath: selectedArtifactPath, targetPacketPath: selectedPacketPath, targetEvidencePacketPath, targetBundlePath, targetReleasePath, targetTagRefPath, targetFeedPath: selectedFeedPath, targetReceiptPath: targetPath }, environment = { PATH: `${value.root}:${process.env.PATH}`, GITHUB_ACTIONS: "true", GITHUB_REPOSITORY: repository, GITHUB_REF: sourceRef, GITHUB_SHA: workflowSHA, GITHUB_WORKFLOW_REF: `${workflow}@${sourceRef}`, RUNNER_ENVIRONMENT: "github-hosted", GITHUB_WORKSPACE: workspace }, command = ["scripts/desktop-update-command.mjs", action, "--accepted-artifact", acceptedArtifactPath, "--accepted-packet", acceptedPacketPath, "--accepted-bundle", bundlePath, "--accepted-release", evidenceReleasePath, "--accepted-tag-ref", evidenceTagRefPath, "--target-artifact", selectedArtifactPath, "--target-packet", selectedPacketPath, "--target-evidence-packet", targetEvidencePacketPath, "--target-bundle", targetBundlePath, "--target-release", targetReleasePath, "--target-tag-ref", targetTagRefPath, "--target-feed", selectedFeedPath, "--target-receipt", targetPath], env = { ...process.env, ...environment };
    const run = (requestedAction = action, overrides: Record<string, string> = {}, mutate: (args: string[]) => string[] = (args) => args, timeout = 10_000) => spawnSync(process.execPath, mutate([...command.slice(0, 1), requestedAction, ...command.slice(2)]), { cwd: process.cwd(), encoding: "utf8", timeout, env: { ...env, ...overrides } });
  ...
  it("rejects noncanonical CLI syntax, alternate history roots, and changed receipts", async () => {
    ...
    const fifo = join(dirname(syntax.selectedFeedPath), "candidate-feed.fifo"); execFileSync("mkfifo", [fifo]); const fifoResult = syntax.run(undefined, {}, (args) => args.map((value) => value === syntax.selectedFeedPath ? fifo : value), 1500); expect(fifoResult.error).toBeUndefined(); expect(fifoResult.status).not.toBe(0);
  ...
  it("rejects cross-release packet substitution and a current target", async () => {
    ...
    const rollback = await transitionAuthorityFixture("rollback"); expect(rollback.run(undefined, {}, (args) => args.map((value) => value === rollback.input.targetBundlePath ? rollback.input.acceptedBundlePath : value)).status).not.toBe(0);
  }, 10_000);
  ...
  it("derives one frozen exact-source/artifact/tree/feed/enclosure/npm packet", async () => {
    ...
    for (const field of ["feedEntry", "productionContract"] as const) { const reordered = JSON.parse(serialized); reordered[field] = Object.fromEntries(Object.entries(reordered[field]).reverse()); expect(() => parseAcceptedDesktopReleasePacket(Buffer.from(`${JSON.stringify(reordered)}\n`))).toThrow("canonical"); }
