...
import { chmodSync, cpSync, existsSync, readFileSync, mkdirSync, mkdtempSync, readdirSync, readlinkSync, renameSync, rmSync, statSync, symlinkSync, truncateSync, writeFileSync } from "node:fs";
...
import { buildAcceptedDesktopTransitionTarget, parseAcceptedDesktopTransitionTarget } from "../scripts/lib/desktop-accepted-transition-target.mjs";
...
describe("canonical accepted Desktop release packet", () => {
  ...
  function packetFixture(sidecar = false, inAppSidecar = false, productionMarkers = byoProductionMarkers, artifactSourceSHA = sourceSHA, options: { version?: string; build?: string; channel?: "stable" | "beta" | "rc"; sequence?: string | null; lightweightTag?: boolean } = {}) {
    ...
    const root = mkdtempSync(join(tmpdir(), "neondiff-packet-")); roots.push(root); mkdirSync(join(root, "accepted-targets")); writeFileSync(join(root, "accepted-targets", ".gitkeep"), "");
...
`); chmodSync(fakeGh, 0o755);
  ...
  async function transitionAuthorityFixture(action: "update" | "rollback" | "reupdate") {
    const beta = packetFixture(false, false, byoProductionMarkers, sourceSHA, { version: "1.1.0-beta.87", build: "11091", channel: "beta", sequence: "87", lightweightTag: true }), rc = packetFixture(false, false, byoProductionMarkers, sourceSHA, { version: "1.1.0-rc.1", build: "12001", channel: "rc", sequence: "1" });
    const betaPacket = await buildAcceptedDesktopReleasePacket(...beta.paths), rcPacket = await buildAcceptedDesktopReleasePacket(...rc.paths), selectedFixture = action === "rollback" ? beta : rc, selectedPacket = action === "rollback" ? betaPacket : rcPacket, currentPacket = action === "rollback" ? rcPacket : betaPacket, previousPacket = action === "update" ? null : selectedPacket;
    const evidence = verifyRetainedTarget(selectedFixture, selectedPacket); expect(evidence.status, evidence.stderr).toBe(0);
    const workspace = join(selectedFixture.root, "protected-workspace"), history = join(workspace, "docs/releases/desktop"), declarations = join(history, "declarations"), targets = join(history, "accepted-targets"), output = join(selectedFixture.root, "transition-output"), base = join(selectedFixture.root, "base-history"); mkdirSync(declarations, { recursive: true }); mkdirSync(targets); mkdirSync(output);
    const betaName = `${betaPacket.tag}.json`, rcName = `${rcPacket.tag}.json`, betaDeclaration = { ...beta.declaration, predecessor: null }, rcDeclaration = { ...rc.declaration, predecessor: betaName }; writeFileSync(join(declarations, betaName), JSON.stringify(betaDeclaration)); writeFileSync(join(declarations, rcName), JSON.stringify(rcDeclaration)); const indexPath = join(history, "index.json"); writeFileSync(indexPath, JSON.stringify({ schemaVersion: 1, status: "retained", declarationDirectory: "declarations", declarationPaths: [betaName, rcName], currentPath: rcName }));
    const packetBytes = (packet: any) => Buffer.from(serializeAcceptedDesktopReleasePacket(packet)), packetPath = (packet: any) => join(targets, `${acceptedDesktopReleasePacketDigest(packet)}.packet.json`), currentPacketPath = packetPath(currentPacket); writeFileSync(currentPacketPath, packetBytes(currentPacket)); let previousTargetPacketPath: string | null = null; if (previousPacket) { previousTargetPacketPath = packetPath(previousPacket); if (!existsSync(previousTargetPacketPath)) writeFileSync(previousTargetPacketPath, packetBytes(previousPacket)); }
    mkdirSync(join(base, "declarations"), { recursive: true }); mkdirSync(join(base, "accepted-targets")); cpSync(declarations, join(base, "declarations"), { recursive: true }); writeFileSync(join(base, "accepted-targets", ".gitkeep"), ""); writeFileSync(join(base, "index.json"), readFileSync(indexPath));
    ...
    const environment = { ...evidence.env, GITHUB_SHA: head, GITHUB_WORKSPACE: workspace }, input = { action, artifactPath: evidence.artifactPath, packetPath: evidence.packetPath, bundlePath: evidence.bundlePath, releasePath: evidence.evidenceRelease, tagRefPath: evidence.evidenceTagRef, currentPacketPath, previousTargetPacketPath }, command = ["scripts/build-desktop-accepted-transition-target.mjs", "--action", action, "--artifact", evidence.artifactPath, "--packet", evidence.packetPath, "--bundle", evidence.bundlePath, "--release", evidence.evidenceRelease, "--tag-ref", evidence.evidenceTagRef, "--current-packet", currentPacketPath, "--output-directory", output]; if (previousTargetPacketPath) command.push("--previous-target-packet", previousTargetPacketPath);
    const run = (requestedAction = action, mutate: (values: string[]) => string[] = (values) => values, overrides: Record<string, string> = {}) => { const values = [...command], at = values.indexOf("--action"); values[at + 1] = requestedAction; return spawnSync(process.execPath, mutate(values), { cwd: process.cwd(), encoding: "utf8", env: { ...environment, ...overrides } }); };
    const build = async () => { const prior = Object.fromEntries(Object.keys(environment).map((key) => [key, process.env[key]])); Object.assign(process.env, environment); try { return await buildAcceptedDesktopTransitionTarget(input); } finally { for (const [key, value] of Object.entries(prior)) if (value === undefined) delete process.env[key]; else process.env[key] = value; } };
    return { action, baseIndexPath: join(base, "index.json"), build, currentPacketPath, evidence, environment, head, history, indexPath, output, previousTargetPacketPath, run, selectedPacket, targets, workspace };
  }
  ...
  it("derives protected update, rollback, and exact re-update authority", async () => {
    for (const action of ["update", "rollback", "reupdate"] as const) {
      const value = await transitionAuthorityFixture(action); expect(spawnSync(process.execPath, ["scripts/validate-desktop-release-declaration.mjs", "--index", value.indexPath, "--base-index", value.baseIndexPath], { cwd: process.cwd(), encoding: "utf8" }).status).toBe(0);
      await expect(value.build()).resolves.toMatchObject({ action });
      const result = value.run(); expect(result.status, result.stderr).toBe(0); const output = JSON.parse(result.stdout), receiptPath = join(value.output, output.targetReceiptFileName), receiptBytes = readFileSync(receiptPath), receipt = parseAcceptedDesktopTransitionTarget(receiptBytes);
      expect(output).toEqual({ action, targetPacketSHA256: acceptedDesktopReleasePacketDigest(value.selectedPacket), targetPacketFileName: `${acceptedDesktopReleasePacketDigest(value.selectedPacket)}.packet.json`, targetReceiptSHA256: createHash("sha256").update(receiptBytes).digest("hex"), targetReceiptFileName: output.targetReceiptFileName });
      expect(receipt).toMatchObject({ action, acceptedTarget: { tag: value.selectedPacket.tag, packetSHA256: acceptedDesktopReleasePacketDigest(value.selectedPacket), artifactSHA256: value.selectedPacket.artifactSHA256, treeSHA256: value.selectedPacket.treeSHA256 }, current: { packetSHA256: createHash("sha256").update(readFileSync(value.currentPacketPath)).digest("hex") }, previouslyAcceptedTargetPacketSHA256: value.previousTargetPacketPath ? createHash("sha256").update(readFileSync(value.previousTargetPacketPath)).digest("hex") : null });
      expect(JSON.stringify(receipt)).not.toMatch(/edSignature|"publicKey"|path|token|customer|config|database|log/i);
      const retainedPacketPath = join(value.targets, output.targetPacketFileName), retainedReceiptPath = join(value.targets, output.targetReceiptFileName); if (!existsSync(retainedPacketPath)) writeFileSync(retainedPacketPath, value.evidence.packetBytes); writeFileSync(retainedReceiptPath, receiptBytes); expect(spawnSync(process.execPath, ["scripts/validate-desktop-release-declaration.mjs", "--index", value.indexPath, "--base-index", value.baseIndexPath], { cwd: process.cwd(), encoding: "utf8" }).status).toBe(0);
    }
  }, 60_000);
  it("rejects relabeling, mutable history, a current target, and receipt replacement", async () => {
    const relabeled = await transitionAuthorityFixture("reupdate"); expect(relabeled.run("update").status).not.toBe(0);
    const current = await transitionAuthorityFixture("reupdate"); expect(current.run(undefined, (values) => values.map((value) => value === current.currentPacketPath ? current.previousTargetPacketPath! : value)).status).not.toBe(0);
    ...
    const replacement = await transitionAuthorityFixture("update"), first = replacement.run(); expect(first.status, first.stderr).toBe(0); expect(replacement.run().status).not.toBe(0); const output = JSON.parse(first.stdout), retainedPacket = join(replacement.targets, output.targetPacketFileName), retainedReceipt = join(replacement.targets, output.targetReceiptFileName); writeFileSync(retainedPacket, replacement.evidence.packetBytes); writeFileSync(retainedReceipt, readFileSync(join(replacement.output, output.targetReceiptFileName))); const acceptedBase = join(dirname(replacement.history), "accepted-base"); cpSync(replacement.history, acceptedBase, { recursive: true }); rmSync(retainedReceipt); const deletion = spawnSync(process.execPath, ["scripts/validate-desktop-release-declaration.mjs", "--index", replacement.indexPath, "--base-index", join(acceptedBase, "index.json")], { cwd: process.cwd(), encoding: "utf8" }); expect(deletion.status).not.toBe(0); expect(deletion.stderr).toMatch(/accepted target history rewritten/i);
  }, 60_000);
