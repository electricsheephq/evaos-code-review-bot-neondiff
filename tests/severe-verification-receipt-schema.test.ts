...
describe("strict severe verification receipt schema", () => {
  ...
  it("rejects duplicate omitted paths regardless of reason code", () => {
    const invalid: SevereVerificationReceipt = { ...receipt(), state: "incomplete", disposition: "suppress", reasonCode: "evidence_incomplete",
      evidence: { files: [{ ...receipt().evidence.files[0], kind: "module", complete: false }], omitted: [{ path: "src/missing.ts", code: "evidence_incomplete" }, { path: "src/missing.ts", code: "not_read" }], complete: false } };
    expect(validate(invalid)).toBe(false);
  });
