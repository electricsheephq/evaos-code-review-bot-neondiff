...
describe("review state store", () => {
  ...
  it("stores ordered latest and independent recent-failure receipts", () => {
    ...
    const hostile = laneReceipt(true, "completed"), reads = { count: 0 }; Object.defineProperty(hostile.counts, "reposScanned", { get: () => reads.count++ === 0 ? 0 : "SECRET_SENTINEL" }); const hostileStore = new ReviewStateStore(join(root, "hostile.sqlite")); expect(hostileStore.recordIssueEnrichmentLaneReceipt({ receipt: hostile as never, runId: "55555555-5555-4555-8555-555555555555", runStartedAt: "2026-08-27T02:00:00.000Z", cycle: 1, recordedAt: "2026-08-27T02:00:01.000Z" })).toBe(true); expect(reads.count).toBe(1); hostileStore.close(); const raw = new DatabaseSync(join(root, "hostile.sqlite"), { readOnly: true }); expect((raw.prepare("select latest_receipt_json from issue_enrichment_receipt_state").get() as { latest_receipt_json: string }).latest_receipt_json).not.toContain("SECRET_SENTINEL"); raw.close();
    const bindingReads = { receipt: 0, runId: 0, runStartedAt: 0, cycle: 0, recordedAt: 0 }, binding = { get receipt() { bindingReads.receipt += 1; return bindingReads.receipt === 1 ? success : "SECRET_SENTINEL"; }, get runId() { bindingReads.runId += 1; return bindingReads.runId === 1 ? "55555555-5555-4555-8555-555555555555" : "SECRET_SENTINEL"; }, get runStartedAt() { bindingReads.runStartedAt += 1; return bindingReads.runStartedAt === 1 ? "2026-08-27T02:00:00.000Z" : "SECRET_SENTINEL"; }, get cycle() { bindingReads.cycle += 1; return bindingReads.cycle === 1 ? 1 : "SECRET_SENTINEL"; }, get recordedAt() { bindingReads.recordedAt += 1; return bindingReads.recordedAt === 1 ? "2026-08-27T02:00:01.000Z" : "SECRET_SENTINEL"; } }; const bindingStore = new ReviewStateStore(join(root, "binding.sqlite")); expect(bindingStore.recordIssueEnrichmentLaneReceipt(binding as never)).toBe(true); expect(bindingReads).toEqual({ receipt: 1, runId: 1, runStartedAt: 1, cycle: 1, recordedAt: 1 }); bindingStore.close(); const bindingDb = new DatabaseSync(join(root, "binding.sqlite"), { readOnly: true }); expect((bindingDb.prepare("select latest_receipt_json from issue_enrichment_receipt_state").get() as { latest_receipt_json: string }).latest_receipt_json).not.toContain("SECRET_SENTINEL"); bindingDb.close();
