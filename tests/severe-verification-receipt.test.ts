...
describe("serialized severe verification receipt", () => {
  ...
  it("proves canonical ordering, strict schema, identity, and exact semantic errors", () => {
    ...
    expect(() => parseSerializedSevereVerificationReceipt(Buffer.concat([Buffer.from([0xef, 0xbb, 0xbf]), Buffer.from(text)]))).toThrow("noncanonical");
    for (const control of ["\u0001", "\u0085", "\u009f"]) expect(() => parseSerializedSevereVerificationReceipt(serial({ evidence: { files: [{ path: `src/${control}x.ts`, kind: "module", sha256: "c".repeat(64), bytes: 1, complete: true }], omitted: [], complete: true } }))).toThrow(/path_invalid|schema_invalid/);
