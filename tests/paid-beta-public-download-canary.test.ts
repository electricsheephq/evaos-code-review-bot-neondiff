...
describe("paid beta public RC canary", () => {
  ...
  it("requires the canonical Sparkle namespace, beta channel, and exact enclosure", () => {
    ...
    expect(run(valid.replace(' sparkle:channel="beta"', ""))).not.toBe(0);
    expect(run(valid.replace('sparkle:channel="beta"', 'sparkle:channel="alpha"'))).not.toBe(0);
    expect(run(valid.replace('sparkle:channel="beta"', 'sparkle:channel="beta" sparkle:channel="beta"'))).not.toBe(0);
