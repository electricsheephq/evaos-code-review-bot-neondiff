...
describe("issue-enrichment receipt classifier", () => {
  it.each([
    ...
    ["lease with failure evidence", { kind: "result", result: result({ summary: counts({ workerSkipped: 1, failed: 1 }) }) }, false, "result_not_ok", "unknown_failure"],
    ["disabled", { kind: "result", result: result({ status: { state: "disabled", blockers: ["issue_enrichment_disabled"] } }) }, true, "disabled", undefined],
    ["disabled without marker", { kind: "result", result: result({ status: { state: "disabled", blockers: [] } }) }, false, "result_not_ok", "unknown_failure"],
    ["disabled with cycle evidence", { kind: "result", result: result({ summary: counts({ issuesSeen: 1 }), status: { state: "disabled", blockers: ["issue_enrichment_disabled"] } }) }, false, "result_not_ok", "unknown_failure"],
