...
function fixture(attestedSourceSHA: string) {
  ...
  const predicate = { schemaVersion: 1, claimClass: "neondiff.desktop.artifact-source-promotion.v1", repository, signerWorkflow: workflow, workflowSourceRef: sourceRef, workflowSourceSHA: workflowSHA, releaseTag: "v1.1.0", artifactSourceSHA: attestedSourceSHA, developerIDTeamID: "TC6MS3T6NN" };
...
describe("Desktop artifact-source promotion attestation", () => {
  ...
  it("verifies and retains one content-addressed bundle under fixed GitHub identity", () => {
    ...
    expect(receipt).toEqual({ verified: true, artifactName: value.artifactName, artifactSHA256: value.artifactSHA256, artifactByteLength: readFileSync(value.artifact).length, artifactSourceSHA: sourceSHA, workflowSourceSHA: workflowSHA, predicateType, bundleSHA256, bundleFileName: `${bundleSHA256}.artifact-source-attestation.json` });
  ...
  it("fails closed for untrusted verification output, runner identity, and unsafe evidence", () => {
    ...
    const mutations = [(value: any) => { value.predicate.repository = "attacker/example"; }, (value: any) => { value.predicate.signerWorkflow = "attacker/example/.github/workflows/release.yml"; }, (value: any) => { value.predicate.workflowSourceSHA = "8".repeat(40); }, (value: any) => { value.predicate.releaseTag = "v1.1.0-rc.1"; }, (value: any) => { value.predicate.developerIDTeamID = "ATTACKER00"; }, (value: any) => { value.predicateType = "https://slsa.dev/provenance/v1"; }];
  ...
  it("keeps promotion authority fixed and verifies before packet construction", () => {
    ...
    expect(builderSource.indexOf("verifyAndRetainDesktopArtifactSourceAttestation")).toBeLessThan(builderSource.indexOf("buildAcceptedDesktopReleasePacket(values.index")); expect(builderSource).not.toMatch(/private.?key|secret/i);
  });
});
