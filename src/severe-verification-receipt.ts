...
function decodeSerialized(input: unknown): string {
  ...
  else if (genuineBytes(input)) { const length = Reflect.apply(getLength, input, []); if (length > MAX_SEVERE_VERIFICATION_RECEIPT_BYTES) fail("cap_exceeded", "serialized receipt exceeds byte cap"); bytes = Buffer.from(Reflect.apply(getBuffer, input, []) as ArrayBuffer, Reflect.apply(getOffset, input, []), length); }
...
function genuineBytes(value: unknown): value is Uint8Array { if (!ArrayBuffer.isView(value)) return false; const proto = Object.getPrototypeOf(value); if (proto !== Uint8Array.prototype && proto !== Buffer.prototype) return false; return !Reflect.ownKeys(value).some((key) => { if (typeof key === "string" && /^\d+$/.test(key)) return false; const descriptor = Object.getOwnPropertyDescriptor(value, key); return Boolean(descriptor && (descriptor.get || descriptor.set || typeof descriptor.value === "function")); }); }
