...
import type { IssueEnrichmentAdmissionDecision } from "./issue-enrichment-admission-decision.js";
...
function readLimits(value: Readonly<Record<string, unknown>>): Limits { const root = plain(value), repos = plain(root.repos), parsed = new Map<string, RepoLimits>(); for (const [repo, raw] of Object.entries(repos)) { const limits = plain(raw), issues = positive(limits.maxIssuesPerCycle), comments = count(limits.maxCommentsPerCycle), burst = positive(limits.maxIssuesPerBurst); if (comments > issues) fail("invalid_limit"); parsed.set(repoKey(repo), { issues, comments, burst }); } const globalIssues = positive(root.globalMaxIssuesPerCycle), globalComments = count(root.globalMaxCommentsPerCycle); if (globalComments > globalIssues) fail("invalid_limit"); return { globalIssues, globalComments, repos: parsed }; }
