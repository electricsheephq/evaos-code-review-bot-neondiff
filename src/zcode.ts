...
export function buildReviewPrompt(input: {
  ...
  const promptPrefix = [
    ...
    ...(isLcmReviewRepository(input.repo) ? [
      ...
      "acceptance_evidence must name concrete inspected paths and the observations supporting your verdict. unresolved_findings must equal the titles of every finding in findings, in the same order, including findings without an inline diff location."
    ] : []),
    "",
...
export async function runZCodeRawJson(input: { cwd: string; prompt: string; cliPath: string; appConfigPath: string; model: string; providerId?: string; timeoutMs?: number; retryMaxRetries?: number }): Promise<string> {
  ...
  const rawResponse = result.status === 0 ? extractZCodeResponse(result.stdout) : ""; const stdout = redactSecrets(result.stdout.replaceAll(zcodeEnv.ZCODE_API_KEY, "[redacted-secret]")); const stderr = redactSecrets(result.stderr.replaceAll(zcodeEnv.ZCODE_API_KEY, "[redacted-secret]"));
  ...
  if (result.status !== 0) throw new Error(`ZCode failed with status ${result.status}: ${stderr || stdout.slice(0, 1000)}`); return rawResponse;
}
