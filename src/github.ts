  ...
  async listIssuesForEnrichment(
    ...
    for (let page = 1; page <= pageLimit; page += 1) {
      ...
      for (const issue of pageIssues) {
        issues.push(issue);
        if (options.qualifyForMinimum && await options.qualifyForMinimum(issue)) qualifiedIssueResults += 1;
        if (options.qualifyForMinimum && minIssueResults > 0 && qualifiedIssueResults >= minIssueResults) return Object.assign(issues, { scanCompletion: "stopped_after_min_issue_results" as const });
      }
      if (chunk.length < perPage) return Object.assign(issues, { scanCompletion: "complete" as const });
      if (!options.qualifyForMinimum && minIssueResults > 0 && issues.length >= minIssueResults) {
