...
function validatePacket(packet: ReviewBenchAdjudicationPacketV1): void {
  ...
  const { packetFingerprint: _ignored, ...basis } = packet;
...
function validateDecisions(
  ...
  for (const [index, decision] of decisions.entries()) {
    requireExactKeys(decision, ["candidateId", "actionability", "severity"], `${label}.decisions[${index}]`);
...
function requireExactKeys(value: unknown, allowed: readonly string[], label: string): asserts value is Record<string, unknown> {
  ...
  const missing = allowed.filter((key) => key !== "severity" && !(key in value));
