...
export async function runIssueEnrichmentCycle(input: {
  ...
  try {
    ...
    const preservedEventReadFailures = new Set<string>();
    const previewIssue = (repo: string, issue: GitHubRelatedIssueOrPull) => { const policy = resolveIssueEnrichmentRepoPolicy(config, repo), allowlists = issueSuggestionAllowlists(policy.suggestions); return buildIssueEnrichmentDryRunOutput({ repo, issue, repoPolicy: policy.repoPolicy, allowedLabels: allowlists.allowedLabels, allowedOwners: allowlists.allowedOwners, maxRelatedRefs: 8, ...renderPolicy }); };
      ...
      ? await collectIssueEnrichmentScan({
          ...
          reader: {
            listIssuesForEnrichment: async (repo, options) => {
              const prepared = new Map<string, GitHubRelatedIssueOrPull>();
              const qualifyForMinimum = async (issue: GitHubRelatedIssueOrPull): Promise<boolean> => {
                const key = issueKey(repo, issue.number);
                const existing = input.force === true ? undefined : input.state.getIssueEnrichmentRecord(repo, issue.number);
                ...
                if (preview.skipped && (!shouldDeferPreservationPreviewToPromotion(preview.reason) || !override?.promotionMaintainers?.length || !input.github.getCollaboratorPermission)) { prepared.set(key, issue); issuesByKey.set(key, issue); return false; }
              ...
              for (const issue of issues) if (!prepared.has(issueKey(repo, issue.number))) await qualifyForMinimum(issue);
              return Object.assign(issues.flatMap((issue) => prepared.has(issueKey(repo, issue.number)) ? [prepared.get(issueKey(repo, issue.number))!] : []), { scanCompletion: issues.scanCompletion });
            }
          },
          dryRun: input.dryRun,
          repos: reposToScan,
