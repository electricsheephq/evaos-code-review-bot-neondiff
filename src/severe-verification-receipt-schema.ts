...
const SAFE_PATH = "^(?!/)(?![A-Za-z]:)(?![\\s\\S]*//)(?![\\s\\S]*(?:^|/)\\.{1,2}(?:/|$))(?![\\s\\S]*[\\\\\\u0000-\\u001f\\u007f-\\u009f\\uD800-\\uDFFF])(?![\\s\\S]*\\/$)[\\s\\S]+$";
    ...
    evidence: {
      ...
      properties: {
        ...
        omitted: { type: "array", maxItems: 64, items: {
          type: "object", additionalProperties: false, required: ["path", "code"], properties: { path, code: { enum: SEVERE_VERIFICATION_CODES } }
        } },
  ...
  ajv.addKeyword({ keyword: "noEvidencePathOverlap", type: "object", schemaType: "boolean", validate: (_schema: boolean, evidence: unknown) => {
    ...
    if (Array.isArray(files)) for (const file of files) {
      if (file && typeof file === "object" && !Array.isArray(file) && (file as { kind?: unknown }).kind === "whole_file") {
        ...
        if (typeof path === "string" && wholeFiles.has(path)) return false;
        if (typeof path === "string") wholeFiles.add(path);
      }
    }
    ...
    if (Array.isArray(omitted)) for (const item of omitted) {
      ...
      if (typeof path === "string" && (wholeFiles.has(path) || omittedPaths.has(path))) return false;
      if (typeof path === "string") omittedPaths.add(path);
    }
    return true;
  } });
