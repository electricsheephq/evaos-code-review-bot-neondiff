...
function snapshotResult(result: unknown): IssueEnrichmentReceiptSnapshot {
  ...
  const statusSnapshot: IssueEnrichmentStatusSnapshot = frozen({
    readable, state: !state.ok || typeof state.value !== "string" ? "unreadable" : state.value === "disabled" ? "disabled" : state.value === "blocked" ? "blocked" : state.value === "ready" ? "ready" : state.value === "dry_run_only" ? "dry_run_only" : "unreadable",
