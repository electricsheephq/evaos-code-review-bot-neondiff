...
function hunkMetadata(value: string | Uint8Array): ChangedHunkMetadata {
  ...
  let bytes: number, data: Buffer;
  if (typeof value === "string") {
    bytes = Buffer.byteLength(value, "utf8");
    if (bytes > MAX_EVIDENCE_BYTES) return { bytes, complete: false, code: "cap_exceeded" };
    if ([...value].some((char) => { const code = char.codePointAt(0)!; return code >= 0xd800 && code <= 0xdfff; })) return { bytes, complete: false, code: "evidence_incomplete" };
    data = Buffer.from(value, "utf8");
  } else {
    try { bytes = Reflect.apply(typedArrayByteLength, value, []) as number; } catch { throw new Error("hunk_invalid"); }
    if (bytes > MAX_EVIDENCE_BYTES) return { bytes, complete: false, code: "cap_exceeded" };
    try {
      ...
      data = Buffer.from(new Uint8Array(buffer, offset, bytes));
    } catch { throw new Error("hunk_invalid"); }
  }
